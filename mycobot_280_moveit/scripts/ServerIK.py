...
class MoveItPlanningServerDemo:
    ...
    def write_angles(self,command):
        ...
        self.arm.set_joint_value_target([command[0]*(math.pi/180), command[1]*(math.pi/180), command[2]*(math.pi/180), command[3]*(math.pi/180), command[4]*(math.pi/180), command[5]*(math.pi/180)])
        self.arm.go()
        #or simply write direct to send angles of the mycobot
    ...
    def write_cartesian_waypoint_stop_and_reverse(self, cartesian_waypoint_x, cartesian_waypoint_y, cartesian_waypoint_z): #x, y, z
        if cartesian_waypoint_x != 0.00000 or cartesian_waypoint_y != 0.00000 or cartesian_waypoint_z != 0.00000:
             self.arm.stop()
             move_cnt = 0
             while move_cnt < 3:
                  move_cnt += 1
                  print("CNT: ", move_cnt)
                  ##self.arm.shift_pose_target(4, -0.2,  self.end_effector_link)#r
                  self.arm.shift_pose_target(3, -0.15,  self.end_effector_link)#z #down
                  self.arm.shift_pose_target(5, +0.35,  self.end_effector_link)#y #right
		  self.arm.go()
                  coords_move_pose = 0
                  if coords_move_pose == 1:
                        waypoints = []
			wpose = self.arm.get_current_pose().pose
		        wpose_start_waypoint = wpose
		 	print("X: ", wpose_start_waypoint.position.x)
			print("Y: ", wpose_start_waypoint.position.y)
			print("Z: ", wpose_start_waypoint.position.z)
			#print("orientation_x: ", wpose_start_waypoint.orientation.x)
			#print("orientation_y: ", wpose_start_waypoint.orientation.y)
			#print("orientation_z: ", wpose_start_waypoint.orientation.z)
			#print("orientation_w: ", wpose_start_waypoint.orientation.w)
			if cartesian_waypoint_x != 0.00000:
			    wpose.position.x += cartesian_waypoint_x #- nach vorne / + nach hinten
			    print("z(x):", wpose.position.x)
			if cartesian_waypoint_y != 0.00000:
			...
			if cartesian_waypoint_z != 0.00000:
			    wpose.position.z += cartesian_waypoint_z #- nach unten #+ nach oben
			    print("y(z):", wpose.position.z)
			#wpose.orientation.x += 0.01
			#wpose.orientation.y += 0.01
			#wpose.orientation.z += 0.01
			#wpose.orientation.w += 0.01
			waypoints.append(copy.deepcopy(wpose))
			(plan, fraction) = self.arm.compute_cartesian_path(waypoints, 0.01, 0)
			display_trajectory = moveit_msgs.msg.DisplayTrajectory()
			display_trajectory.trajectory_start = self.arm.get_current_state()
		  ...
		  # reverse movement
		  ##self.arm.shift_pose_target(4, -0.2,  self.end_effector_link)#r
                  self.arm.shift_pose_target(3, +0.15,  self.end_effector_link)#z up
                  self.arm.shift_pose_target(5, -0.35,  self.end_effector_link)#y left
		  self.arm.go()
                  if coords_move_pose == 1:
                        waypoints = []
		        wpose = self.arm.get_current_pose().pose
	...
	#print("type Mov:", type(moving_coords[0]))
    ...
    def moving(self):
        ...
        # set base link
            ...
            #target_pose.header.frame_id = self.reference_frame
            #target_pose.header.stamp = rospy.Time.now()
            #target_pose.pose.position.x = 0.05665243474230666
            #target_pose.pose.position.y = 0.17303518325040498
            #target_pose.pose.position.z = 0.281491542368593
            #target_pose.pose.orientation.x = -0.9949604354724465
            #target_pose.pose.orientation.y = -0.02264101310680033
            #target_pose.pose.orientation.z = 0.002704886131136401
            #target_pose.pose.orientation.w = 0.09764117963746247
            # set current state
            #self.arm.set_start_state_to_current_state()
            # set pose target
            #self.arm.set_pose_target(target_pose, self.end_effector_link)
            ...
            # execute trajectory
            #self.arm.execute(traj, wait=True)
            joint_val = self.arm.set_joint_value_target([130*(math.pi/180), -0.25634597039563134, -25*(math.pi/180), -39*(math.pi/180), 0.0, 0.0])
            print("yolo_Ai_pose")
            #joint_val = self.arm.set_joint_value_target([1.617018399512283, -0.6633657767868899, -0.4398857938561518, -0.678831488809992,  0.0008023626494878453, 0.0009043064243817252])
            self.arm.go()
        ...
        #print("SET_MARKER")
        #self.pub_marker(0.2, 0.2, 0.2)

        if arm_yolo_pos == 0:
            self.arm.shift_pose_target(5, 0.2,  self.end_effector_link)#y
            #self.arm.shift_pose_target(3, 0.2,  self.end_effector_link)#z
            self.arm.go()
        #else: #arm_yolo_pos
            #self.arm.shift_pose_target(5, 0.2,  self.end_effector_link)#y
            #self.arm.shift_pose_target(3, 0.2,  self.end_effector_link)#z
            ...
            while True:
                ...
                while True:
                    try:
                        ...
                        if cmd == float(2222.2): #read is gripper moving
                        ...
                        elif cmd == float(22222.2): #cartesian waypoint stop and reverse
                            write_cartesian_wayp = [values[1], values[2], values[3], values[4], values[5], values[6]]
                            write_cartesian_wayp_round = [round(each_val, 5) for each_val in write_cartesian_wayp]
                            write_cartesian_wayp_round_x, write_cartesian_wayp_round_y, write_cartesian_wayp_round_z = write_cartesian_wayp_round[:3]
                            print("write_cartesian_wayp_round_x: ", write_cartesian_wayp_round_x)
                            print("write_cartesian_wayp_round_y: ", write_cartesian_wayp_round_y)
                            print("write_cartesian_wayp_round_z: ", write_cartesian_wayp_round_z)
                            self.write_cartesian_waypoint_stop_and_reverse(write_cartesian_wayp_round_x, write_cartesian_wayp_round_y, write_cartesian_wayp_round_z)
